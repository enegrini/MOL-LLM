...
concate_pytree_fn = lambda list_of_trees, axis=0: tree.tree_map(
    lambda *xs: jnp.concatenate(list(xs), axis), *list_of_trees
)
...
#         init_value=0.0, peak_value=peak_lr, warmup_steps=warmup_steps, decay_steps=decay_steps, end_value=end_lr
#     )
...
def get_partial_optimizer(params, trainable_key_list, untrainable_key_list, optimizer=None):
    ...
    partial_optimizer = optax.multi_transform({"optimizer": optimizer, "zero": optax.set_to_zero()}, param_labels)
...
class DataSet:
    ...
    def add_to_buffer(self, new_data_list):
        buffer = [
            jnp.concatenate(
                [jax.device_put(d, jax.devices("cpu")[0]), jax.device_put(nd, jax.devices("cpu")[0])], axis=0
            )[-self.buffer_cap :, ...]
